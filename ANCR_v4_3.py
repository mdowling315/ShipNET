...
def i_arrange(g, la):
    ...
    # print p_loc
    for comp in p_loc:
        if 'un' in list(la['components'][comp]['loc'].values()):
            #calculate locations
            comp_loc[comp] = {}
            p_t = 0.0
...
def i_dist(g, la, sys, e,key='loc'):
    ...
    if 'un' in list(i_locs_ini.values()):
        # if unallocated add super node
        i_unal = True
        i_edges = []
        for x in list(i_locs_ini.keys()):
    ...
    if 'un' in list(j_locs_ini.values()):
        # if unallocated add super node
        j_unal = True
        j_edges = []
        for x in list(j_locs_ini.keys()):
    ...
    # Get current distribution for locations    TODO fix problem here
    p_r = dict.fromkeys(list(g_super.nodes()) + list(g_super.edges()), 0.0)  # probability dictionary
    ##p_r = dict.fromkeys(g_super.nodes() + g_super.edges(), 0.0)  # probability dictionary

    #     print 'i',i_locs
    #     print 'j',j_locs
    #
    for i_loc, belief_i in i_locs.items():
        for j_loc, belief_j in j_locs.items():
...
def update_resistance(la, location_list, draw_list, score_list, feasibility_threshold):
    ...
    for comp, comp_d in upd_la['components'].items():
    ...
    for sys, ploc in zip(LA_I['systems'], plotlocs):
        ...
        for component in LA_I['components']:
            if component in LA_I[sys].nodes():
                for l, prob in LA_I['components'][component]['i'].items():
                    size[l] += prob
...
def plot_setups(g, LA, cutoff=0.0, scale=1.0, elev=15, angle=-75, factor=2.0):
    ...
    for sys, ploc in zip(LA_I['systems'], plotlocs):
        ...
        for component in LA_I['components']:
            if component in LA_I[sys].nodes():
                for l, prob in LA_I['components'][component]['loc'].items():
                    # if prob=='un':
